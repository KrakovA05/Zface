...
// Контекстный рефрейминг — по одной строке на каждое слово из DAILY_WORDS
export const DAILY_WORDS_CONTEXT = [
  'иногда это — пространство для чего-то нового',
  'сигнал, а не приговор',
  'в ней есть сила — важно её заметить',
  'значит, ты что-то делал. это честно',
  'даже в нём можно быть рядом с собой',
  'не безразличие, а перегрузка',
  'это значит, что-то важное было',
  'что-то не то — хорошо, что ты замечаешь',
  'быть потерянным — не стыдно',
  'это одна из самых честных эмоций',
  'он есть у всех. у тебя тоже — и ок',
  'иногда остановиться — единственный выход',
  'тело говорит то, что слова не могут',
  'самое тёмное — не значит последнее',
  'за ней часто прячется что-то важное',
  'чувствовать — не значит быть виноватым',
  'ты заслуживаешь мягкости — в первую очередь от себя',
  'значит, что-то важное не было замечено',
  'она есть. это хорошо',
  'говорит о том, чего на самом деле хочешь',
  'мозг пытается защитить. иногда он перебарщивает',
  'когда всё отдал — надо что-то взять обратно',
  'ощущение не равно реальности',
  'это честность, а не слабость',
  'она проходит. не сразу — но проходит',
];
...
export const LEVEL_DATA = {
  green: {
    label: '🟢 Зелёный',
    color: '#4CAF50',
    text: 'Ты держишься. Это честный результат.',
    emoji: '🌿',
  },
  yellow: {
    label: '🟡 Жёлтый',
  ...
  red: {
    label: '🔴 Красный',
    color: '#F44336',
    text: 'Сейчас тяжело. Ты это замечаешь — и это важно. Ты не один.',
    emoji: '🌪',
  },
};
...
export const MOTIVATORS = [
  ...
  'Иногда ничего не делать — это тоже выбор. Вполне нормальный',
  'Сегодня можно просто существовать. Этого достаточно',
];
...
export const ACHIEVEMENT_GROUPS = [
  {
    ...
    achievements: [
      ...
      { id: 'ten_tests',    icon: 'analytics-outline',   label: 'Самоанализ',  desc: 'Десять раз — это уже не случайность',    hidden: false },
      { id: 'twenty_tests', icon: 'telescope-outline',   label: 'Глубже',      desc: 'Двадцать тестов. Ты смотришь честно',    hidden: false },
    ],
  },
  {
    id: 'daily',
    ...
    achievements: [
      ...
      { id: 'checkin_7',     icon: 'calendar-outline',    label: 'Неделя честности', desc: 'Семь чекинов подряд. Ты замечаешь себя',      hidden: false},
      { id: 'daily_7',       icon: 'chatbox-outline',     label: 'Неделя',      desc: 'Семь дней подряд. Ты здесь — и это важно',  hidden: false },
      { id: 'daily_14',      icon: 'star-outline',        label: 'Две недели',  desc: 'Четырнадцать дней подряд. Ты здесь каждый день', hidden: false },
      { id: 'daily_30',      icon: 'ribbon-outline',      label: 'Месяц',       desc: 'Тридцать дней. Это уже часть тебя',          hidden: false },
      { id: 'streak_14',     icon: 'flame-outline',       label: 'Две недели',       desc: 'Четырнадцать дней в приложении подряд',       hidden: false},
    ],
  },
  {
    id: 'voice',
    label: 'Голос',
    achievements: [
      { id: 'profile_done',       icon: 'person-outline',              label: 'Личность',       desc: 'Заполнил статус и добавил аватар. Тебя теперь видно', hidden: false },
      { id: 'first_post',         icon: 'create-outline',              label: 'Голос',          desc: 'Написал пост. Кто-то прочитал и почувствовал',  hidden: false},
      ...
      { id: 'thought_reactions_5',icon: 'heart-outline',               label: 'Тебя услышали', desc: 'Пять реакций на твою мысль',                   hidden: false },
      { id: 'first_reaction',     icon: 'hand-left-outline',           label: 'Поддержал',     desc: 'Первая реакция на чужую мысль',                hidden: false },
      { id: 'first_letter',       icon: 'mail-outline',                label: 'В темноте',     desc: 'Написал анонимное письмо кому-то. Это смелость', hidden: false },
    ],
  },
  {
    id: 'connections',
    ...
    achievements: [
      ...
      { id: 'helper_5',     icon: 'sunny-outline',       label: 'Рядом',          desc: 'Пять людей сказали что ты им помог',            hidden: false},
      { id: 'helper_20',      icon: 'bonfire-outline',     label: 'Маяк',          desc: 'Двадцать. Ты стал чем-то важным для кого-то',  hidden: false },
      { id: 'letter_replied', icon: 'chatbubble-outline', label: 'Услышан',       desc: 'Кто-то ответил на твоё письмо. Тебя заметили',  hidden: false },
    ],
  },
  {
    id: 'depth',
    ...
    achievements: [
      ...
      { id: 'psych_all',      icon: 'prism-outline',    label: 'Полная картина',   desc: 'Все 8 психотестов пройдены хотя бы раз',        hidden: false},
      { id: 'breathing_first',icon: 'sync-outline',     label: 'Выдох',            desc: 'Первая дыхательная сессия',                      hidden: false },
      { id: 'breathing_10',   icon: 'water-outline',    label: 'Дышу',             desc: 'Десять сессий дыхания',                          hidden: false},
      { id: 'fish_first',     icon: 'fish-outline',     label: 'Рыбак',            desc: 'Первая пойманная рыба',                          hidden: false },
      { id: 'fish_rare',      icon: 'diamond-outline',  label: 'Редкость',         desc: 'Поймал редкую или легендарную рыбу',             hidden: false},
    ],
  },
];
